$(document).ready(function(){
  ...
  $('.colors-large a').click(function(){return false;});
	...
	$('.add_to_cart-shop').click(function(){
		var idCard = $(this).find('.add_id_cart').val();
		$.ajax({
	...
	$(document).change(function(){
		$('.table-responsive input').on('click',function(){
			var checkvalue = [];
			$.each($("input[name='checkbox_product']:checked"), function(){   
				checkvalue.push($(this).val());
			});	
			if(checkvalue.length >0){
				$('.btn_delete').addClass('display-block');
			}
			else{
				$('.btn_delete').removeClass('display-block');
			}
			$('.deleteAll').val(checkvalue)		
	...
	$(document).ready(function(){
		...
		$('.btn_delete').click(function(){
			var data = $('.deleteAll').val();
			var check = confirm('Bạn chắc chắn muốn xóa không')
			if(check == true){
				$.ajax({
				url: 'http://finalproject.test/admin/product/delete',
				dataType: "html",
				data: {
					data: data,
				},
		...
		$('.input-search').keyup(function(){
			var data = $(this).val();
			$.ajax({
				url: 'http://finalproject.test/admin/product/search',
				dataType: "html",
				data : {
					data : data,
				},
